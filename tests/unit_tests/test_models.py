...
@pytest.mark.parametrize(
    ...
    [
        (
            ["actor.global_batch_size=1000"],
            r"16384 action-chunk samples.*global_batch_size \(1000\)",
        ),
...
def test_openwam_sft_recipe_builds_on_cpu():
    """The SFT preset defers device placement to FSDP."""
    import hydra
...
def test_openwam_get_model_honors_load_to_device(monkeypatch, openwam_recipe):
    ...
    def fake_from_checkpoint(cls, **kwargs):
        ...
        policy.value_head = torch.nn.Linear(8, 1)
