...
- 完整 PPO 只在 LIBERO 上跑过。RoboTwin 的评测适配已接入（``evaluations/robotwin/robotwin_<task>_openwam_eval.yaml``，见 :doc:`../../evaluations/guides/robotwin`），尚未在真实 RoboTwin 环境里跑过验证。
